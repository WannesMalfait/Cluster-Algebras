\chapter{Quantum cluster algebras}\label{sec:quantum_cluster_algebras}
\chaptertoc
...
Let us now first fix some notation, similar to that of the previous chapter.
\begin{notation}
	In what follows, $\bbK$ will be a field (of any characteristic) and $N \in \bbZ_{> 0}$
	an integer. We denote by $\bbK^\times$\index[symbols]{K times@$\bbK^\times$} the multiplicative
	...
	\end{equation*}
	\index[symbols]{B A C@$\prescript{B}{}{}A^C$}%
	%
	with special cases
	\begin{equation*}
...
\begin{theorem}[\protect{\cite[Theorem 6.2]{GoodearlWarfield2004NoncommutativeNR}}]
...
set, we write $\Fract(R)$\index[symbols]{fract R@$\Fract(R)$} for the corresponding ring of
fractions, and say that $R$ is an \emph{Ore domain}\index{Ore!domain}. For example, the
...
$b^k$\index[symbols]{b k@$b^k$} for the $k$-th column of $\tB$. We will denote with
$[b^k]_{+}$\index[symbols]{b k plusminus@$[b^k]_{\pm}$} and $[b^k]_{-}$ the positive and
...
\end{equation*}
\index[symbols]{Omega q@$\Omega_\bbq(f,g)$}%
%
...
\begin{definition}\label{def:compatible_pair}
	...
	Let $\tB \in M_{N \times \ex}(\bbZ)$ be an integer matrix, and let $\bbq \in
		M_N(\bbK^\times)$ be a multiplicatively skew-symmetric matrix. Let $\wt{\bbt}\in M_{\ex
				\times N}(\bbZ)$\index[symbols]{t tilde@$\wt{\bbt}$} be the matrix with entries
	\begin{equation*}
...
We now look at the mutation of the compatible pairs. Recall from
...
\end{equation*}
\index[symbols]{Y f@$Y^{(f)}$}%
%
where
\begin{equation*}
	\mcS_{\mathbf{r}}(f) \defeq \prod_{i < j}r_{ij}^{-f_if_j}.
\end{equation*}
\index[symbols]{S r@$\mcS_\bbr(f)$}%
%
...
\begin{remark}
	...
	\end{align*}
	We'll use the notation $\mathbf{r}(M)$\index[symbols]{r M@$\bbr(M)$} to denote the matrix of the toric frame $M$.
\end{remark}
%
...
\begin{definition}
	A \emph{quantum seed}\index{seed!quantum}\index{quantum!seed} is a pair $(M, \tB)$ consisting of a toric frame $M$
...
the \emph{quantum cluster algebra}\index{algebra!cluster!}\index{cluster
	algebra!quantum}\index{quantum!cluster algebra} $\mcA(M, \tB, \binv)_\bbK$\index[symbols]{Amc M B
	inv@$\mcA(M, \tB, \binv)_\bbK$} completely analogously to the classical setting.
...
\emph{upper quantum cluster algebra}\index{algebra!cluster!quantum upper} $\mcU(M, \tB,
	\binv)_\bbK$ is then defined as the intersection
\begin{equation*}
	\mcU(M, \tB, \binv) \defeq \bigcap_{(M', \tB')} \mcT\mcA_{(M', \tB')} ,
\end{equation*}
\index[symbols]{Umc M B inv@$\mcU(M, \tB, \binv)_\bbK$}%
%
...
phenomenon (\cref{thm:laurent_phenomenon}), which we will refer to as the \emph{quantum
	Laurent phenomenon}\index{quantum!Laurent phenomenon}\index{Laurent!phenomenon!quantum}.
...
A quantum version of positivity also holds. \Textcite{Davison2018PositivityQuantumCA}
...
Fix $q \in \bbC^\times$ not a root of unity, and let $\mcO_q(M_{m \times
			k}(\bbC))$\index[symbols]{O q M n@$\mcO_q(M_{n \times m}(\bbK))$} be the noncommutative algebra
...
This algebra is called the \emph{quantized matrix coordinate
	algebra}\index{algebra!quantized matrix coordinate}\index{quantum!matrix algebra}.

The \emph{quantum Grassmannian}\index{Grassmannian!quantum}\index{quantum!Grassmannian}
$\mcO_q(\Gr_{k,m}(\bbC))$\index[symbols]{O q Gr@$\mcO_q(\Gr_{k,m}(\bbC))$} is the subalgebra of
$\mcO_q(M_{k\times m}(\bbC))$ generated by all the $k \times k$ \emph{quantum
	minors}\index{minor!quantum}\index{quantum!minor}. For $1 \leq i_1 < \dotsb < i_k \leq
	m$ and $J = \{i_1, \dotsc, i_k\}$, the $k \times k$ quantum minor
$\Delta^J_q$\index[symbols]{Delta J q@$\Delta^J_q$} is defined as
\begin{equation*}
...
We will now illustrate the quantum cluster algebra structure on $\mcO_q(\Gr_{2,
...
We now look at the special case of $\mcO_q(\Gr_{2,4}(\bbC))$. The frozen variables are
...
Although the classical cluster algebra structure and the quantum cluster algebra
