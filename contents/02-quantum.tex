...
All of the algebras that we have discussed thusfar have been commutative. Indeed, a
...
class is given by deformations of universal envelopping algebras and coordinate
...
be dedicated to describing how one can generalise cluster algebras to the
...
The goal of this section is to generalise cluster algebras to the noncommutative
...
\begin{equation*}
	\mcA_\bbq = \frac{\bbK \langle Y_1 , \dots, Y_N\rangle}{(Y_i Y_j = r_{ij}Y_j Y_i)},
\end{equation*}
...
\begin{equation}\label{eq:quantum_torus}
	\mcT_\bbq = \frac{\bbK \langle Y_1^{\pm 1} , \dots, Y_N^{\pm 1}\rangle}{(Y_i Y_j = r_{ij}Y_j Y_i)}.
\end{equation}
%
...
\begin{remark}
	...
	$\mcT_\bbq$ as a \emph{uniparameter}\index{uniparameter} quantum tori in this case.
\end{remark}
%
...
The idea is that we want to define a new ring $R[E\inv] = \{re\inv \mid e \in E, r\in
	R\}$ for some subset $E \subseteq R$ of a (noncommutative) ring $R$. In order for this
...
fractions is a little bit more involved.
...
(as an algebra), we have $e R = R e$ for all $e \in E$. Furthermore, all the nonzero
elements in $\mcA_\bbq$ are regular. Thus, $E$ is an Ore set.
...
set, we write $\Fract(R)$ for the corresponding ring of fractions. The quantum cluster
variables will live in the division algebra $\Fract(\mcT_\bbq)$.
...
algebra is determined by an initial seed, consisting of a matrix and the intitial
...
(\cref{eq:exchange_relation_quiver}) to the noncommutative setting. This will give us
the intuition needed to arrive at the right definition. Take $k \in \ex$ and write
$b^k$ for the $k$-th column of $\tB$. We will denote with $[b^k]_{+}$ and $[b^k]_{-}$
the positive and negative parts of $b^k \in \bbZ^N$. Thus
\begin{align*}
...
\end{equation}
%
By convention we take all the products to be in increasing index\footnote{If this feels
	arbitrary, that is because it is. This will be addressed by switching over to toric
	frames.}, i.e., $Y^f = Y_1^{f_1}Y_2^{f_2} \cdots Y_N^{f_N}$ for $f \in \bbZ^N$. For
this definition to work, the variables $Y_1, \dots, Y_{k-1}, Y'_k, Y_{k+1}, \dots, Y_N$
should now be contained in another quantum torus $\mcT_{\bbq'}$. Thus, for each $j \in
...
We now look to the mutation of the compatible pairs. Recall that the mutation of $\tB$
...
the definition of $\bbq'$ is independent of the choice of sign. We then define the
\emph{mutation in direction $k \in \ex$}\index{mutation!of compatible pairs} of the
compatible pair $(\bbq, \tB)$ to be the pair $(\mu_k (\bbq),\mu_k (\tB) )$. That this
yields another compatible pair is a straightforward verification.
\begin{proposition}[\protect{\cite[Proposition 2.6]{GoodearlYakimov2017QCA}}]\label{prop:mutation_preserves_good_things}
	...
	compatible, the principal par of $\mu_k(\tB)$ is skew-symmetrizable, and the
...
\begin{align*}
	...
	= Y_K.
\end{align*}
As desired, we have the following multiplication rule for $f,g \in \bbZ^N$:
\begin{equation*}
...
\end{remark}
%
We are now ready to define the quantum analogs of seeds, mutations and cluster
algebras.
\begin{definition}
...
Mutation of quantum seeds is defined by the analogon of
...
\end{proposition}
\begin{remark}
	Note again that the definition defines the mutation of $\bbr$ and not of the matrix $\bbq$. However, we still have $\mu_k(\bbr)^{\cdot 2} = \mu_k(\bbq)$ where $\mu_k(\bbq)$ is defined via \cref{eq:def_mutation_q}, so that the definition behaves as expected.
\end{remark}
...
\end{equation*}
%
Then the \emph{upper quantum cluster algebra}\index{algebra!upper quantum cluster}
$\mcU(M, \tB, \binv)_\bbK$ is defined as the intersection
\begin{equation*}
	\mcU(M, \tB, \binv) = \bigcap_{(M', \tB')} \mcT\mcA_{(M', \tB')} ,
\end{equation*}
%
where the intersection runs over all quantum seeds $(M', \tB')$ mutation-equivalent to
the initial seed $(M, \tB)$. We note two special cases. If $\binv = \emptyset$, then
$\mcT\mcA_{(M, \tB)} = \mcA_{\bbr(M)^{\cdot 2}}$, while if $\binv = [1, N] \setminus
	\ex$, then $\mcT\mcA_{(M, \tB)} = \mcT_{\bbr(M)^{\cdot 2}}$. This explains the
notation. We now have the following generalization of the Laurent phenomenon
(\cref{thm:laurent_phenomenon}), which we will refer to as the quantum Laurent
phenomenon.
...
\begin{example}
	Let $A_1^q$ be the $\bbK$-algebra generated by $x_1, x_2$ such that $x_2 x_1 - q x_1 x_2 = 1$. This algebra is known as the \emph{quantised Weyl algebra}\index{algebra!quantised Weyl}. It is an iterated Ore extension, with presentation
	\begin{equation*}
...
\begin{lemma}\label{lem:equivariance_mutations}
	Let $\mcF$ be a division algebra and $\mcH$ a group acting on $\mcF$ by algebra automorphisms. Assume that $(M, \tB)$ is a quantum seed for $\mcF$ such that $M(f)$ is an $\mcH$-eigenvector for all $f \in \bbZ^N$, and that $\chi_{M(b^j)} = 1$ for all columns $b^j$ of $\tB$. Then all mutations of the seed $(M, \tB)$ share the same properties.
\end{lemma}
\begin{proof}
	...
	Similarly for $j \neq k$, if $b_{kj} = 0$ then $\chi_{M'((b')^j)} = \chi_{M(b^j)} = 1$.
	If $b_{kj} \neq 0$, then
	\begin{equation*}
