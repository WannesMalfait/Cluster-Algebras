...
\begin{notation}
	...
	an integer. We denote by $\bbK^\times$\index{K times@$\bbK^\times$} the multiplicative
...
$b^k$\index{b k@$b^k$} for the $k$-th column of $\tB$. We will denote with
$[b^k]_{+}$\index{b k plusminus@$[b^k]_{\pm}$} and $[b^k]_{-}$ the positive and
...
\begin{remark}
	...
	\end{align*}
	We'll use the notation $\mathbf{r}(M)$\index{r M@$\bbr(M)$} to denote the matrix of the toric frame $M$.
\end{remark}
%
...
the \emph{quantum cluster algebra}\index{algebra!quantum cluster}\index{cluster
	algebra!quantum}\index{quantum!cluster algebra} $\mcA(M, \tB, \binv)_\bbK$\index{A M B
...
phenomenon (\cref{thm:laurent_phenomenon}), which we will refer to as the \emph{quantum
	Laurent phenomenon}\index{quantum!Laurent phenomenon}\index{Laurent!quantum
	-phenomenon}.
...
Fix $q \in \bbC^\times$ not a root of unity, and let $\mcO_q(M_{m \times
			k}(\bbC))$\index{O q M n@$\mcO_q(M_{n \times m}(\bbK))$} be the noncommutative algebra
...
$\mcO_q(\Gr_{k,m}(\bbC))$\index{O q Gr@$\mcO_q(\Gr_{k,m}(\bbC))$} is the subalgebra of
...
$\Delta^J_q$\index{Delta J q@$\Delta^J_q$} is defined as
\begin{equation*}
