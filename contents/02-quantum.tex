...
Let $\bbr \in M_N(\bbK^\times)$ be a \emph{multiplicatively
	skew-symmetric}\index{skew-symmetric!multiplicatively} matrix, i.e., $r_{ij} =
	r_{ji}\inv$ for all $i,j \in [1, N]$. We then write $\mcT_\bbr$ for the quantum torus
\begin{equation*}
	\mcT_\bbr = \frac{\bbK \langle Y_1^{\pm 1} , \dots, Y_N^{\pm 1}\rangle}{(Y_i Y_j = r_{ij}Y_j Y_i)}.
\end{equation*}
%
\begin{remark}

	In most examples, $\bbr$ will be of the form $(q^{\Lambda_{ij}})$ for some
	skew-symmetric matrix $\Lambda \in M_N(\bbZ)$. We will refer to $\mcT_r$ as a
	\emph{uniparameter}\index{uniparameter} quantum tori in this case.
\end{remark}
%
Before moving on to the definition of quantum seeds and mutations, we take a look at
rational actions by tori. These will be used extensively in the context of CGL
extensions. \medskip
...
multiplicatively skew-symmetric matrix $\bbr \in M_N(\bbK^\times)$. We will combine the
data of $\bbr$ together with the data of the cluster variables in one object, called a
\emph{toric frame}.

Fix a multiplicatively skew-symmetric matrix $\bbr \in M_N(\bbK^\times)$. In the
quantum torus $\mcT_\bbr$, we have that $Y_i Y_j = r_{ij}Y_j Y_i =
	\prescript{e_i}{}{}\bbr^{e_j} Y_j Y_i$. Write $Y^f = Y_1 ^f{f_1} \cdots Y_N^{f_N}$ for
$f \in \bbZ^N$. It would now be desirable to also have that
\begin{equation*}
	Y^f Y^g = \prescript{f^T}{}{}\bbr^g Y^g Y^f
\end{equation*}
for all $f, g \in \bbZ^N$. This is already true... \textbf{TODO: motivation for why move to based quantum torus...}

Associated to any such matrix, is a skew-symmetric bicharacter \footnote{With a
	bicharacter we mean a map from a direct product, such that each of the component maps
	is a group character.}:
\begin{equation*}
...
quantum torus $\mcT_{\mathbf{r}^{\cdot 2}}$, where
\begin{equation*}
...
So, what is this extra compatibility? In analogy with
\cref{eq:exchange_relation_quiver} we want to define $\mu_k(M)$ in such a way that
$\mu_k(M)(e_j) = M(e_j)$ for $j \neq k$ and
\begin{equation*}
...
\begin{proof}
	...
	\end{equation*}
	%
	We can thus apply the part we already proved with the roles of $\tau$ and $\tau'$ swapped. This
	immediately resolves case (a), since it is symmetric in $\tau$ and $\tau'$. It remains
	to show that the conclusions of (b) for $\tau$ also hold for $\tau'$. To do this, we
	must carefully compare $\bbr_\tau$ and $\bbr_{\tau'}$.
...
\begin{proof}
	...
	The proof for the other implication is analogous. The difference is that we now have to consider the case $\tau(k) > \tau(k+1)$ in \cref{prop:mutation_adjacent_permutations} (b). In this case we obtain
	\begin{equation*}
	...
	\end{align*}
	%
	The rest of the proof can then be taken over literally from the first case, by swapping $\tau$ and $\tau'$.
...
\begin{proof}
	...
	\end{equation*}
	%
	We now apply \cref{lem:inductive_step_btautau} once more, to conclude that $\tB_{\sigma, \tau'}$
...
\begin{proof}[Proof of \cref{thm:gy_main_result} (a), (b) and (c)]
	...
	\cref{lem:inductive_step_btautau}, swapping the roles of $\tau$ and $\tau'$ if needed. Then, since mutation is involutive, every $\tau \in
		\Xi_N$ can be obtained by a sequence of one-step mutations from $\id$ due to
	\cref{prop:eta_invariance} and the uniqueness of part (a), we obtain that all the
	quantum seeds associated to $\Xi_N$ are mutation-equivalent to each other.
\end{proof}
	...
	\begin{enumerate}[(a)]
		...
		\item Again, one inclusion is clear. It suffices to show $v \in R[y_k\inv \mid k \in \binv]$
		      ...
		      choose $y$ to be the same for each $\tau \in \Gamma_N$, and hence $vy \in
			      R[E\inv_\tau]$ for all $\tau \in \Gamma_N$ (cf.
		      \cite[Lemma~6.1~(c)]{GoodearlWarfield2004NoncommutativeNR}). Thus, $vy\in R$ by part
		      (d), and $v \in R[y_k\inv \mid k \in \binv]$ as we needed to show.
	\end{enumerate}
\end{proof}
